function plotDirTune(trigData, cellNum)
...
f1=figure('Color', 'w', 'Position', [100 400 560 420]);
for dir=1:8
    ...
    if sum(idx)>0
        ...
        hold on
        plot(trigData.periTime,mean(trigData.Traces{cellNum,1}(idx,:)), 'Color', [1 0 0], 'LineWidth', 2)
        plot(trigData.periTime,mean(trigData.Traces{cellNum,1}(idx,:))+std(trigData.Traces{cellNum,1}(idx,:)), 'Color', [1 0 0], 'LineWidth', 1, 'LineStyle', '--')
        plot(trigData.periTime,mean(trigData.Traces{cellNum,1}(idx,:))-std(trigData.Traces{cellNum,1}(idx,:)), 'Color', [1 0 0], 'LineWidth', 1, 'LineStyle', '--')
        hold off
    end
